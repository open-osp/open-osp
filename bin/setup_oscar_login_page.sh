...
sed -e "s|supportLink=.*|supportLink=$SUPPORT_LINK|g" -e "s|supportText=.*|supportText=$SUPPORT_TEXT|g" -e "s|supportName=.*|supportName=$SUPPORT_NAME|g" -e "s|clinicLink=.*|clinicLink=$CLINIC_LINK|g" -e "s|clinicText=.*|clinicText=$CLINIC_TEXT|g" -e "s|clinicName=.*|clinicName=$CLINIC_NAME|g" -e "s|tabName=.*|tabName=$TAB_NAME|g" docker/tomcat_oscar/conf/templates/login_page_template.txt > login_page_template.txt.tmp  
